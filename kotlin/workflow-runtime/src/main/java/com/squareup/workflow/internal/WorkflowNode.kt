...
internal class WorkflowNode<PropsT, StateT, OutputT : Any, RenderingT>(
  ...
  private fun <T> createWorkerNode(
    ...
    if (diagnosticListener != null) {
      ...
      diagnosticListener
          .onWorkerStarted(workerId, diagnosticId, key, worker.toString())
    }
    val workerChannel = launchWorker(worker, workerId, diagnosticId, diagnosticListener)
